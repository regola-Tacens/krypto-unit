      ...
      {currencies.map((currency, index) => {
        ...
        <tr className="currenciesTable__tr" key={currency.id}>
          ...
          <td className="currenciesTable__price">
            ${toComaNbr(currency.price)}
          </td>
          {isNovice || isExpert ? null : (
            <td
              className={
                currency.percent_change_1h < 0
                  ? "currenciesTable__percent negative"
                  : "positive currenciesTable__percent"
              }
            >
              {currency.percent_change_1h.toFixed(2)}%
            </td>
          )}
          {isNovice ? null : (
            <td
              className={
                currency.percent_change_24h < 0
                  ? "currenciesTable__percent negative"
                  : "currenciesTable__percent positive"
              }
            >
          ...
          >
            {currency.percent_change_7d.toFixed(2)}%
          </td>
          {isNovice || isExpert ? null : (
            <td className="currenciesTable__volume24">
              ${toComaNbr(currency.volume_24h)}
            </td>
          )}
          {isNovice || isExpert ? null : (
            <td className="currenciesTable__circulatingSupply">
              ${toComaNbr(currency.circulating_supply)}
            </td>
          )}
          {isNovice ? null : (
            <td className="currenciesTable__maxSupply">
              ${toComaNbr(currency.max_supply)}
            </td>
          )}
          <td className="currenciesTable__MktCap">
            ${toComaNbr(currency.market_cap)}
          </td>
